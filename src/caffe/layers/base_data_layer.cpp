...
void BasePrefetchingLabelmapDataLayer<Dtype>::LayerSetUp(
  ...
  for (int i = 0; i < PREFETCH_COUNT; ++i) {
    ...
    prefetch_[i].superpixelabelmap_.mutable_cpu_data();
    }
#ifndef CPU_ONLY
  if (Caffe::mode() == Caffe::GPU) {
    for (int i = 0; i < PREFETCH_COUNT; ++i) {
      ...
      prefetch_[i].superpixelabelmap_.mutable_gpu_data();
    }
  }
#endif
...
void BasePrefetchingLabelmapDataLayer<Dtype>::Forward_cpu(
  ...
  // Copy the labels.
  caffe_copy(batch->superpixelabelmap_.count(), batch->superpixelabelmap_.cpu_data(),
